...
class LudoBoard:
    ...
    def __init__(self, master):
        ...
        self.frame = tk.Frame(master, width=Board.PANEL_WIDTH, height=Board.PANEL_HEIGHT, bg=Color.INDIGO)
        self.Quit = tk.Button(master, text='QUIT', command=master.quit, relief=tk.RAISED, width=20, height=2)
        self.title_bar = tk.Label(master, text=Text.HEADER, fg=Color.DEFAULT, bg=Color.INDIGO, font=(None, 40), relief=tk.RAISED)
    ...
    def path(self):
        ...
        for i, j in Board.POSITIVE_V:
            ...
            self.draw_circle(i + 0.7, j + 0.7, i + 1.3, j + 1.3, Color.AQUA)
        for j, i in Board.POSITIVE_H:
            if i > j:
                self.draw_rectangle(j + 0.5, i + 0.5, j + 1.5, i + 1.5, Color.GREEN, 1)
            else:
                self.draw_rectangle(j + 0.5, i + 0.5, j + 1.5, i + 1.5, Color.BLUE, 1)
            self.draw_circle(j + 0.7, i + 0.7, j + 1.3, i + 1.3, Color.AQUA)
